...
echo ""
echo -e "${YELLOW}${BOLD}ローカル → Supabase 同期${NC}"
echo ""
...
echo -e "  転送元: ${BOLD}ローカル${NC} (sailing_score_db)"
echo -e "  転送先: ${BOLD}Supabase${NC}"
echo ""
echo -e "${YELLOW}警告: 同じIDのデータはローカルの内容で上書きされます。${NC}"
echo ""
...
python3 "$BACKEND_DIR/sync.py" to_cloud

EXIT_CODE=$?
if [ $EXIT_CODE -eq 0 ]; then
    echo ""
    echo -e "${GREEN}${BOLD}同期完了: ローカル → Supabase${NC}"
else
    echo ""
